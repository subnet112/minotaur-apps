    ...
    try {
      ...
      if (store.walletMode === 'external' && walletClient && store.contractAddress && order.order_id) {
        try {
          ...
          // Relayer settlement — gasless. The relayer pays gas and sends the
          // on-chain executeIntent TX, pulling the input token via
          // transferFrom. This covers native ETH too: by the time we get
          // here the user has already wrapped ETH→WETH and approved WETH
          // (the action button walks Wrap → Approve → Sign before submit), so
          // the order pulls WETH like any other ERC-20. The contract's
          // msg.value auto-wrap branch is intentionally unused — the relayer
          // can't attach msg.value, and we don't make the user broadcast the
          // swap themselves.
